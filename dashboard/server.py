"""
dashboard/server.py — AEGIS Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "AEGIS Uploads",
        Path.home() / "Documents" / "AEGIS Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'AEGIS-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"AEGIS Dashboard Port {port}"
        prog_rule  = "AEGIS Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="aegis_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until AEGIS is run as Administrator.")
...
def _ensure_certs() -> bool:
    ...
    key_p = certs / "aegis.key"
    crt_p = certs / "aegis.crt"
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "AEGIS Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "AEGIS"),
        ])
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        certs = BASE_DIR / "config" / "certs"
        return (certs / "aegis.key").exists() and (certs / "aegis.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in AEGIS to get a new QR code.</p>
</div></body></html>""")
...
<p>Connecting to AEGIS…</p>
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in AEGIS UI to get the QR code.")
        try:
            await uvicorn.Server(cfg).serve()
        except OSError as e:
            if "address already in use" in str(e).lower():
                print(f"[Dashboard] Port {PORT} in use - remote control may be unavailable. Try closing other apps using port {PORT}.")
            else:
                raise
