...
def get_assistant_name() -> str:
    """Return the configured assistant name, or 'AEGIS' if not set."""
    return load_api_keys().get("assistant_name", "AEGIS") or "AEGIS"
...
def save_assistant_config(assistant_name: str, user_name: str) -> None:
    ...
    if CONFIG_FILE.exists():
        try:
            ...
            data = {}
    data["assistant_name"] = assistant_name.strip() or "AEGIS"
...
def get_input_device() -> str:
    ...
    Always return system default so device selection follows PC connections dynamically."""
    # Ignore saved preference for dynamic device selection
    return ""
...
# ── Files save location ─────────────────────────────────────────────────────
# Where AEGIS writes generated files (code, screenshots, documents…).
# Defaults to the user's home directory so the app still works when installed
# next to a read-only exe (e.g. Program Files).

def get_files_save_dir() -> str:
    """Folder AEGIS saves generated/downloaded files into."""
    return (load_api_keys().get("files_save_dir") or "").strip() or str(Path.home())


def save_files_save_dir(path: str) -> None:
    """Persist the user-chosen files-save location (empty → home dir)."""
    _patch_config(files_save_dir=(path or "").strip() or str(Path.home()))