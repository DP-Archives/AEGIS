...
def _load_plugin_file(path: Path) -> PluginRecord:
    """Import *path* as a plugin module and validate it.

    Shared by discovery (discover_plugins) and the in-app installer
    (install_plugin_file) so both use the exact same validation pipeline.
    """
...
def install_plugin_file(src_path, plugins_dir: Path) -> tuple[bool, str]:
    """Copy a .py plugin file from anywhere on disk into plugins_dir and validate it.

    Returns (ok: bool, message: str). Powers the in-app "Install Plugin" button —
    the user picks a file, the app copies it into the plugins folder, validates it
    with the normal pipeline, and rolls the copy back if it is broken.
    """
    src = Path(src_path)
    if src.suffix.lower() != ".py":
        return False, "Only .py plugin files are supported."
    if not src.exists():
        return False, f"Plugin file not found: {src}"
    if src.name.startswith("_"):
        return False, "Plugin file names cannot start with '_' (reserved)."

    plugins_dir.mkdir(parents=True, exist_ok=True)
    dest = plugins_dir / src.name
    if dest.resolve() == src.resolve():
        return True, f"'{src.name}' is already installed in the plugins folder."

    try:
        shutil.copy2(src, dest)   # overwrites on reinstall — keeps the folder clean
    except Exception as e:
        return False, f"Could not copy plugin file: {e}"

    try:
        rec = _load_plugin_file(dest)
    except Exception as e:
        try:
            dest.unlink()
        except Exception:
    ...
    return True, f"Plugin '{rec.name}' installed from '{src.name}'."
