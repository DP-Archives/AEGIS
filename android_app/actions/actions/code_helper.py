...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    base = _files_save_dir()
    if output_path:
        p = Path(output_path)
        return p if p.is_absolute() else base / p
    ext = ext_map.get((language or "python").lower(), ".py")
    return base / f"dp_code{ext}"


def _files_save_dir() -> Path:
    """Saved-files folder chosen by the user at setup ('files_save_dir')."""
    try:
        cfg = json.loads(API_CONFIG_PATH.read_text(encoding="utf-8"))
        d = (cfg.get("files_save_dir") or "").strip()
        if d:
            return Path(d)
    except Exception:
        pass
